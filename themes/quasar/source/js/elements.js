  ...
  searchInputs.forEach(function (input) {
    input.addEventListener('keyup', function (evt) {
      ...
      if (evt.keyCode == 13) { // <- Enter
        ...
        var response = window.search(keywords);

        // Update result list's header
        var searchResultHeader = document.querySelector('#qwb-search-results .qwb-modal-header > h2');
        ...
        var searchResultList = document.querySelector('#qwb-search-results .qwb-modal-content');
        searchResultList.innerHTML = '<p>Sorry, no results...</p>';
        if (response.length > 0) {
          searchResultList.innerHTML = response.map(function (entry) {
            return '<p><a href="' + entry.url + '">' + entry.title + '</a>' + '<br/>(' + entry.keywords.length + ' matches)</p>';
          }).join('');
        }

        // Open the modal for search results
        window.location = ("" + window.location).replace(/#.*$/, '') + '#qwb-search-results';
      }
    });
  })
